...
function updateTaskAssignmentDisplay(taskId, assignedUserId, assignedUserName) {
    ...
    let assignmentDisplay = taskTextContainer.querySelector('.assignment-display');
    ...
    if (assignedUserId && assignedUserName) {
        ...
        if (!assignmentDisplay) {
            ...
            assignmentDisplay.className = 'assignment-display flex items-center space-x-2 mt-1 ml-1';
            assignmentDisplay.innerHTML = `
                <div class="user-avatar-small" 
                     style="width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-weight: 600; font-size: 9px; background-color: ${color};" 
                     title="${assignedUserName}">
                    ${initials}
                </div>
                <span class="text-xs text-[var(--text-secondary)]">${assignedUserName}</span>
            `;
            // Insert before the reminder display if it exists, otherwise append
            ...
            if (taskTextDiv) {
                if (reminderDisplay) {
                    taskTextDiv.parentNode.insertBefore(assignmentDisplay, reminderDisplay);
                } else {
