"""
Management command to send feedback request emails to all users.

Usage:
    # Send to all users (subscribed only)
    python manage.py send_personal_feedback_request

    # Send to specific user by email
    python manage.py send_personal_feedback_request --email user@example.com

    # Send to multiple users
    python manage.py send_personal_feedback_request --email user1@example.com --email user2@example.com

    # Dry run (don't actually send emails)
    python manage.py send_personal_feedback_request --dry-run
    
    # Include all users (including unsubscribed)
    python manage.py send_personal_feedback_request --all-users
"""
...
class Command(BaseCommand):
    help = 'Send feedback request emails to all users'
    ...
    def handle(self, *args, **options):
        ...
        if specific_emails:
            ...
            # Send to all users regardless of subscription status
        ...
        if dry_run:
            ...
            for user in users:
                ...
                self.stdout.write(f'  {subscribed} {user.email} ({user.first_name or "No name"}) - Tier: {user.tier}')
            return
